// +AMGD
// $Id: geoid.cpp 4 2012-08-24 02:28:17Z mike $
// file:  geoid.cpp
// author:  See below.
 ...
 * ABSTRACT
 *
 *    The purpose of GEOID is to support conversions between WGS84
 *    ellipsoid heights and WGS84 geoid heights.
 *
 *
 * ERROR HANDLING
 *
 ...
 */
{
  int items_read = 0;
  char* file_name = 0;
  char* path_name = getenv( "GEOID_DATA" );
...
/*  Build a File Name, including specified or default path:                   */
  ...
  if (path_name != NULL)
  {
      file_name = (char *)calloc( strlen( path_name ) + 11, sizeof( char ) );
...
/*
 *  15 Nov 09 MJ Pfauth
 *  Only initialize the EGM96 conversion.
 */
 ...
 */
{
  ...
  double south_lat, north_lat, west_lon, east_lon;
  long error_code = 0;
  ...
  { /* no errors */
    ...
    height_ne = height_buffer[ index + 1 ];

    index = ( post_y + 1 ) * num_cols + post_x;
    height_sw = height_buffer[ index ];
    height_se = height_buffer[ index + 1 ];

    west_lon = post_x * scale_factor;
    east_lon = west_lon + scale_factor;
    north_lat = 90 - ( post_y * scale_factor );
    south_lat = north_lat - scale_factor;

    /*  Perform Bi-Linear Interpolation to compute Height above Ellipsoid:        */

    if( longitude_dd < 0.0 )
      delta_x = ( longitude_dd + 360.0 - west_lon ) / ( east_lon - west_lon );
    else
      delta_x = ( longitude_dd - west_lon ) / ( east_lon - west_lon );
    delta_y = ( latitude_dd - south_lat ) / ( north_lat - south_lat );
    ...
    *delta_height = height_sw * w_sw + height_se * w_se + height_ne * w_ne 
      + height_nw * w_nw;
  }
 ...
 */
{
  ...
  { /* no errors */
    ...
    {
      temp_offset_x = ( int )( offset_x * 2.0 );
      temp_offset_y = ( int )( offset_y * 2.0 + 1.0e-11 );
      if ( ( temp_offset_x + 1 ) == EGM96_COLS )
        temp_offset_x--;
      if ( ( temp_offset_y + 1 ) == EGM96_ROWS )
        temp_offset_y--;

      index = ( int )( temp_offset_y * EGM96_COLS + temp_offset_x );
      height_nw = height_buffer[ index ];
      height_ne = height_buffer[ index + 2 ];

      index = ( int )( ( temp_offset_y + 1 ) * EGM96_COLS + temp_offset_x );
      height_sw = height_buffer[ index ];
      height_se = height_buffer[ index + 2 ];

    }
    else if( scale_factor == SCALE_FACTOR_1_DEGREE )
    {
      temp_offset_x = ( int )( offset_x * 4.0 );
      temp_offset_y = ( int )( offset_y * 4.0 + 1.0e-11 );
      if ( ( temp_offset_x + 1 ) == EGM96_COLS)
        temp_offset_x--;
      if ( ( temp_offset_y + 1 ) == EGM96_ROWS)
        temp_offset_y--;
    ...
    {
      temp_offset_x = ( int )( offset_x * 8.0 );
      temp_offset_y = ( int )( offset_y * 8.0 + 1.0e-11 );
      if ( ( temp_offset_x + 1 ) == EGM96_COLS)
        temp_offset_x--;
      if ( ( temp_offset_y + 1 ) == EGM96_ROWS)
        temp_offset_y--;
      ...
      index = ( int )( temp_offset_y * EGM96_COLS + temp_offset_x );
      height_nw = height_buffer[ index ];
      height_ne = height_buffer[ index + 8 ];

      index = ( int )( ( temp_offset_y + 1 ) * EGM96_COLS + temp_offset_x );
      height_sw = height_buffer[ index ];
      height_se = height_buffer[ index + 8 ];
    }
    else
    {
      index = post_y * num_cols + post_x;
 ...
 */
{
  ...
  error_code = NS_Interpolate( Latitude, Longitude, scale_factor, 
			       num_cols, num_rows, egm96_height_grid, &delta_height );
  *Geoid_Height = Ellipsoid_Height - delta_height;
...
long Convert_Ellipsoid_To_MSL_EGM84_10D_BL_Height( double Latitude,
                                                   ...
                                                   double *Geoid_Height )
/*
 * The function Convert_Ellipsoid_To_MSL_EGM84_10D_BL_Height converts
 * the specified WGS84 ellipsoid height at the specified geodetic
 * coordinates to the equivalent geoid height, using the EGM84 gravity
 * model and the bilinear interpolation method..
 *
 *    Latitude            : Geodetic latitude in radians           (input)
 *    Longitude           : Geodetic longitude in radians          (input)
 *    Ellipsoid_Height    : Ellipsoid height, in meters            (input)
 ...
 */
{
  ...
  error_code = BL_Interpolate( Latitude, Longitude, SCALE_FACTOR_10_DEGREES, 
			       EGM84_COLS, EGM84_ROWS, egm84_height_grid, 
			       &delta_height );
  *Geoid_Height = Ellipsoid_Height - delta_height;
...
long Convert_Ellipsoid_To_MSL_EGM84_10D_NS_Height( double Latitude,
                                                   ...
                                                   double *Geoid_Height )
/*
 * The function Convert_Ellipsoid_To_MSL_EGM84_10D_NS_Height converts
 * the specified WGS84 ellipsoid height at the specified geodetic
 * coordinates to the equivalent geoid height, using the EGM84 gravity
 * model and the natural spline interpolation method..
 *
 *    Latitude            : Geodetic latitude in radians           (input)
 *    Longitude           : Geodetic longitude in radians          (input)
 *    Ellipsoid_Height    : Ellipsoid height, in meters            (input)
 ...
 */
{
  ...
  error_code = NS_Interpolate( Latitude, Longitude, SCALE_FACTOR_10_DEGREES,
			       EGM84_COLS, EGM84_ROWS, egm84_height_grid, 
			       &delta_height );
  *Geoid_Height = Ellipsoid_Height - delta_height;
...
long Convert_Geoid_To_Ellipsoid_Height( double Latitude,
                                        ...
                                        double *Ellipsoid_Height )
/*
 * The function Convert_Geoid_To_Ellipsoid_Height converts the
 * specified WGS84 geoid height at the specified geodetic coordinates
 * to the equivalent ellipsoid height, using the EGM96 gravity model
 * and the bilinear interpolation method.
 *
 *    Latitude            : Geodetic latitude in radians           (input)
 *    Longitude           : Geodetic longitude in radians          (input)
 ...
 */
{
  ...
  error_code = BL_Interpolate( Latitude, Longitude, SCALE_FACTOR_15_MINUTES, 
			       EGM96_COLS, EGM96_ROWS, egm96_height_grid, 
			       &delta_height );
...
long Convert_MSL_EGM96_VG_NS_To_Ellipsoid_Height( double Latitude,
                                                  ...
                                                  double *Ellipsoid_Height )
/*
 * The function Convert_MSL_EGM96_VG_NS_To_Ellipsoid_Height converts
 * the specified WGS84 geoid height at the specified geodetic
 * coordinates to the equivalent ellipsoid height, using the EGM96
 * gravity model and the natural spline interpolation method.
 *
 *    Latitude            : Geodetic latitude in radians           (input)
 *    Longitude           : Geodetic longitude in radians          (input)
 *    Ellipsoid_Height    : Ellipsoid height, in meters            (input)
 ...
 */
{
  ...
  error_code = NS_Interpolate( Latitude, Longitude, scale_factor, num_cols, 
			       num_rows, egm96_height_grid, &delta_height );
  *Ellipsoid_Height = Geoid_Height + delta_height;
...
long Convert_MSL_EGM84_10D_BL_To_Ellipsoid_Height( double Latitude,
                                                   ...
                                                   double *Ellipsoid_Height )
/*
 * The function Convert_MSL_EGM84_10D_BL_To_Ellipsoid_Height converts
 * the specified WGS84 geoid height at the specified geodetic
 * coordinates to the equivalent ellipsoid height, using the EGM84
 * gravity model and the bilinear interpolation method.
 *
 *    Latitude            : Geodetic latitude in radians           (input)
 *    Longitude           : Geodetic longitude in radians          (input)
 *    Ellipsoid_Height    : Ellipsoid height, in meters            (input)
...
long Convert_MSL_EGM84_10D_NS_To_Ellipsoid_Height( double Latitude,
                                                   ...
                                                   double *Ellipsoid_Height )
/*
 * The function Convert_MSL_EGM84_10D_NS_To_Ellipsoid_Height converts
 * the specified WGS84 geoid height at the specified geodetic
 * coordinates to the equivalent ellipsoid height, using the EGM84
 * gravity model and the natural spline interpolation method.
 *
